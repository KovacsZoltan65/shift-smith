...
This module spec MUST comply with:

- `.kiro/specs/_tenancy_rules.md`

Key tenancy constraints for this module:

- Tenant = TenantGroup (NOT Company)
- All tenant-scope data MUST be scoped by `company_id`, and Company MUST belong to a TenantGroup
- All cache keys MUST be tenant-aware: `tenant:{tenant_group_id}:...`
- No naked `DB::table()` or unscoped `Model::query()`; Repository layer enforcement is mandatory
- Multi-DB ready: do not assume all data is in one database
