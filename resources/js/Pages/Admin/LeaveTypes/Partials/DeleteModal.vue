...
const submit = async () => {
    ...
    if (!id) {
        error.value = "Nincs kivalasztott szabadsag tipus.";
        return;
    }
    ...
    try {
        await LeaveTypeService.destroy(id);
        emit("deleted", "Szabadsag tipus torolve.");
        close();
    } catch (requestError) {
...
<template>
    <Dialog
        ...
        modal
        header="Szabadsag tipus torlese"
        :style="{ width: '30rem' }"
        :closable="!deleting"
        :dismissableMask="!deleting"
    >
        <p class="text-sm text-slate-700">
            Biztosan torolni szeretned ezt a szabadsag tipust:
            <strong>{{ leaveType?.name ?? leaveType?.code ?? "#" }}</strong>
        </p>
        ...
        <template #footer>
            <div class="flex justify-end gap-2">
                ...
                <Button
                    ...
                    data-testid="leave-type-delete-confirm"
                    @click="submit"
                />
            </div>
        </template>
