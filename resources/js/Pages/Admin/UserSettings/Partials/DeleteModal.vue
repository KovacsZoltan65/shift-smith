...
const submit = async () => {
    ...
    try {
        ...
        emit("deleted", data?.message ?? "User setting törölve.");
        close();
    } catch (err) {
...
<template>
    <Dialog :visible="modelValue" modal header="User setting törlése" :style="{ width: '30rem' }" @update:visible="emit('update:modelValue', $event)">
        <div class="space-y-3">
