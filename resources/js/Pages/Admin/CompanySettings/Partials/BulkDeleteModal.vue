...
const submit = async () => {
    ...
    try {
        ...
        emit("deleted", data?.message ?? trans("company_settings.messages.bulk_deleted_success"));
        close();
    } catch (err) {
...
<template>
    <Dialog :visible="modelValue" modal :header="$t('company_settings.dialogs.bulk_delete_title')" :style="{ width: '30rem' }" @update:visible="emit('update:modelValue', $event)">
        <div class="space-y-3">
            <p class="text-sm text-gray-700">{{ $t("company_settings.dialogs.bulk_delete_confirm", { count }) }}</p>
